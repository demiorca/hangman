...
word_list = ['год', 'человек', 'время', 'дело', 'жизнь', 'день', 'рука', 'раз', 'работа', 'слово', 'место', 'лицо',
            'друг', 'глаз', 'вопрос', 'дом', 'сторона', 'страна', 'мир', 'случай', 'голова', 'ребёнок', 'сила', 'конец',
            ...
            'двор', 'писатель', 'самолет', 'объём', 'род', 'солнце', 'вера', 'берег', 'спектакль', 'фирма', 'способ',
            ...
            'работник', 'обеспечение', 'офицер', 'фамилия', 'предел', 'выборы', 'учёный', 'бутылка', 'бой', 'теория',
            ...
            'ракета', 'серия', 'шутка', 'вершина', 'выпуск', 'кулак', 'лед', 'торговля', 'нефть', 'молодёжь', 'цифра',
...
def play(word):
    ...
    print(display_hangman(abs(tries - 6)))

    first_last = input('Отображать первую и последнюю букву? Напишите "да" или "нет": ').lower()

    while first_last != 'да' or first_last != 'нет':
        if first_last in ['да', 'нет']:
            if first_last == 'да':
                word_completion = list(word_completion)
                word_completion[0], word_completion[-1] = word[0], word[-1]
                word_completion = ''.join(word_completion)
                break
            elif first_last == 'нет':
                break
        else:
            first_last = input('Вы ввели что-то другое. Напишите "да" или "нет": ').lower()

    print(f'Загаданное слово: {" ".join(list(word_completion))}')
    ...
    while tries:
        ...
        if letter.isalpha():
            if letter in guessed_letters:
                print('Вы уже называли эту букву, напишите другую')
            elif letter in guessed_words:
                print('Вы уже называли это слово, напишите другое')
            else:
                guessed_letters.append(letter) if len(letter) == 1 else guessed_words.append(letter)
                if letter in word:
                    if len(letter) == 1:
                        word_completion = list(word_completion)
                        idx = 0
                        for i in range(word.count(letter)):
                            idx = word.find(letter, idx)
            ...
            print('Вы ввели что-то другое, нужно именно букву русского алфавита, либо слово')
...
def game():
    ...
    play(word)
    question = input('Хотите сыграть ещё? Напишите "да" или "нет": ').lower()
    while question != 'нет':
        if question == 'да':
            word = get_word(word_list)
            play(word)
            ...
            question = input('Вы ввели что-то другое. Напишите "да" или "нет": ').lower()
